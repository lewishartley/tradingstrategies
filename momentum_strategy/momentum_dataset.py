...
end_date = pd.read_sql("SELECT * FROM grouped_bars", con = engine)["t"].values[-1]
full_grouped_bars = pd.read_sql("SELECT * FROM grouped_bars", con = engine)
full_grouped_bars = full_grouped_bars.set_index("t")
full_grouped_bars.index = pd.to_datetime(full_grouped_bars.index)
##

trading_calendar = calendar.schedule(start_date =full_grouped_bars.index[0].strftime("%Y-%m-%d"), end_date = end_date).resample('ME').first()["market_open"].dt.strftime("%Y-%m-%d")
last_trading_calendar = calendar.schedule(start_date = full_grouped_bars.index[0].strftime("%Y-%m-%d"), end_date = end_date).resample('ME').last()["market_open"].dt.strftime("%Y-%m-%d")
...
for first_day in first_trading_days:
    ...
    for ticker in trading_tickers:
        ...
        first_day_close = ticker_data['c'].resample('ME').first()
        last_day_close = ticker_data['c'].resample('ME').last()
